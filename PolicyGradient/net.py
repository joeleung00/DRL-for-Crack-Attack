...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels, blocks_size, activation="relu", block=ResidualBlock, *args, **kwargs):
        ...
        self.finalize_value = nn.Sequential(
            conv_bn(self.blocks_size, self.out_channels, conv1x1),
            activation_func(self.activation),
        )
        
        self.finalize_prob = nn.Sequential(
            conv_bn(self.blocks_size, self.out_channels, conv1x1),
            activation_func(self.activation),
        )
    ...
    def forward(self, x):
        x = self.gate(x)
        x = self.blocks(x)
        v = self.finalize_value(x)
        p = self.finalize_prob(x)
        return p, v
...
class Decoder(nn.Module):
    ...
    def forward(self, p, v):
        v = self.value_decode(v.view(v.size()[0], -1))
        p = self.prob_decode(p.view(p.size()[0], -1))
