...
output_path = "./plot/sac_small_board2.txt"
        ...
        if step >= args.start_learn and step % args.train_freq == 0:
            ...
            if step == args.start_learn:
                if len(replay_memory) > 0:
                    agent.train(replay_memory, args.init_train_times)
            else:
                agent.train(random.sample(replay_memory, args.train_data_size), args.total_epoch)
...
def main():
    ...
    parser.add_argument('--alpha_lr', type=float, required=True,
                        help='how important of entropy')
    
    parser.add_argument('--soft_tau', type=float, required=True,
                        help='soft tau update network')
    
    parser.add_argument('--init_log_alpha', type=float, required=True,
                        help='soft tau update network')
...
#                         help='num of process')
    ...
    parser.add_argument('--init_train_times', type=int, required=True,
                        help='train_freq')
