...
class Decoder(nn.Module):
    def __init__(self, in_features, n_classes, blocks_sizes):
        ...
        self.decode_advantage = nn.Sequential(
        ...
        self.decode_value = nn.Sequential(
            nn.Linear(in_features, blocks_sizes[0]),
            *[nn.Linear(i, j) for (i, j) in zip(blocks_sizes, blocks_sizes[1:])],
    ...
    def forward(self, x):
        advantage = self.decode_advantage(x.view(x.size()[0], -1))
        value = self.decode_value(x.view(x.size()[0], -1))
