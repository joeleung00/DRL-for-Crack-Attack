...
from policy_net import ResidualNet
...
class PolicyAgent:
    ...
    def inference(self, board_list):
        ...
        with torch.no_grad():
            if len(onehot_boards) > 10000:
           
                all_Q_values = torch.zeros((0, ROW_DIM * COLUMN_DIM)).cuda()
                batches = Data.DataLoader(onehot_boards, batch_size=10000, num_workers=10)
    ...
    def extract_q_values(self, outputs, actions_indexes):
        return outputs.gather(1, actions_indexes.view(len(actions_indexes), -1))
