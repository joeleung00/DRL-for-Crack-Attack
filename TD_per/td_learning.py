...
output_path = "./plot/per_small_board2.txt"
    ...
    for step in outer:
        ...
        if step >= args.start_learn and step % args.train_freq == 0:
            if count > 0:
                ...
                count = 0
            if step == args.start_learn:
                experience = replay_memory.sample(args.start_learn, beta=agent.beta)
            else:
                experience = replay_memory.sample(args.train_data_size, beta=agent.beta)
            
            boards, choices, rewards, next_boards, weights, batch_idxes = experience

            td_errors = agent.train((boards, choices, rewards, next_boards, weights))
...
def main():
    ...
    parser.add_argument('--initial_beta', type=float, required=True,
                        help='soft tau update network')
