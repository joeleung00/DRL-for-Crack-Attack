...
class DQNAgent:
    def __init__(self, args, debug=False):
        ...
        if debug:
            ...
            if args != None:
                ...
                self.beta_increase = (1.0 - self.beta) / training_times / 5
    ...
    def MSELoss(self, predicted_q_values, target_q_values, weights):
        loss = weights.unsqueeze(-1) * (predicted_q_values - target_q_values)**2
        return loss.mean()
    
    def calculate_loss(self, states, rewards, actions, next_states, weights):
        with torch.no_grad():
        ...
        loss = self.MSELoss(predicted_q_values, target_q_values, weights)
        return loss, td_error
        
        
    def train(self, replay_memory):
        dataloader = DataLoader(self.args.batch_size, replay_memory)
        _, _, kkk, _, _ = replay_memory
        ...
        for epoch in pbar:
            ...
            for step, data in enumerate(train_iter):
                states, actions, rewards, next_states, weights = data
                states = states.cuda()
                ...
                loss, td_error = self.calculate_loss(states, rewards, actions, next_states, weights)
                if epoch == (self.args.total_epoch - 1):
                    all_td_errors.extend(td_error.tolist())
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                loss, _ = self.calculate_loss(states, rewards, actions, next_states, weights)
